...
public class ParticipationService {
    ...
    public PaginationDTO<Participation> getMyParticipation(int pageNum) {
        ...
        Page<Participation> result =
                participations.findByUser(usersService.getCurrentUser(), pageable);
     ...
     * @return created participation if successful. Else, null if there is already participation
     *         with reward same as rewardId, and belong to current user
     */
    public Participation createParticipation(Long rewardId) {
        ...
        if (participations.findByRewardAndUser(reward, currentUser).isPresent()) {
            log.error("Participation record already exists for Reward id " + rewardId
                    + " for user id " + currentUser.getAccountId());
            return null;
        }
        ...
        log.info("Created Participation with rewardId " + rewardId + " and userId "
                + currentUser.getAccountId());
        return participations.save(participation);
    }

    /**
     * If RewardType = stamp, Add stamp to the Participation object If RewardType = point, add
     * {reward.goal} number of points to the Users object
     * 
